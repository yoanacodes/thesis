...
<template>
  <v-container class="mb-12" id="processWrapper">
    <v-row id="projectTitle" justify="center" align="center" class="mt-4">
      <h2 class="text-overline font-weight-light">Is Food Making You Sick?</h2>
    ...
    <v-row class="dataSources">
      <span
        >Data Sources: <br /><a
          href="https://www.globaldietarydatabase.org/"
          target="_blank"
          >Global Dietary Database</a
        ><br />
        <a href="https://www.healthdata.org/gbd/2019" target="_blank"
          >Global Burden of Diseases</a
        >
      </span>
    </v-row>
...
export default {
  ...
  computed: {
    ...
    healthFiltersLength() {
      if (
        typeof this.filters.healthFilters === "function" ||
        !this.filters.healthFilters.length
      )
        return "84";
      let filtered = this.filters.healthFilters.filter(
        (f) => !f.hasOwnProperty("header")
      );
      return filtered.length;
    },
    nutritionFiltersLength() {
      if (
        typeof this.filters.nutritionFilters === "function" ||
        !this.filters.nutritionFilters.length
      )
        return "47";
      let filtered = this.filters.nutritionFilters.filter(
        (f) => !f.hasOwnProperty("header")
      );
    ...
    concatenateAboutString() {
      let about =
        `<p><i>Is Food Making You Sick?</i> is exploring the relationship between diet and health in 184 countries through the analysis and visualization of extensive data on nutritional factors and diseases. The project aims to study and visualize these relationships by juxtaposing ` +
        this.nutritionFiltersLength +
        ` nutrients and ` +
        this.healthFiltersLength +
          ...
          sketches: [
            {
              ...
              description:
                "After several considerations, the UI was sketched before starting development, in order to map the functionalities to the development process and plan it better.",
              width: "960px",
              layout: "side",
            },
          ],
